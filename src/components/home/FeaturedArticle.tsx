...
import { FaCalendarAlt, FaEye, FaRegBookmark } from 'react-icons/fa';
...
const FeaturedArticle: React.FC<Props> = ({ featuredReviews, isLoading }) => {
      ...
      <div className="container mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-12 gap-4">
            ...
            <div className="col-span-12 text-start lg:col-span-6 row-span-2 mb-4 lg:mb-0 cursor-pointer group">
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group">
                <div className="relative aspect-[16/9] overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 z-10">
                    <span className="bg-blue-600 text-white text-xs uppercase font-bold py-1.5 px-3 rounded-md">
                      {mainFeatured.category?.name || 'Tech'}
                    </span>
                  </div>

                  <div className="absolute top-4 right-4 z-10">
                    <span className="flex items-center gap-1 bg-black/70 backdrop-blur-sm text-white px-3 py-1.5 rounded-md text-xs font-medium">
                      <BsLightningChargeFill className="text-yellow-400" /> Editor's Choice
                    </span>
                  </div>

                  <button className="absolute bottom-4 right-4 w-10 h-10 bg-white/20 backdrop-blur-sm hover:bg-white/30 rounded-full flex items-center justify-center transition-colors z-10">
                    <FaRegBookmark className="text-white" />
                  </button>
                </div>

                <div className="p-5">
                  <Link href={`/reviews/${mainFeatured.slug}`}>
                  ...
                  <p className="text-gray-600 dark:text-gray-300 mb-4 line-clamp-2">
                    {mainFeatured.excerpt || 'Check out this featured review with the latest insights on this tech product.'}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center justify-between">
                    <div className="flex items-center mb-2 md:mb-0">
                      {[...Array(5)].map((_, i) => (
                        <AiFillStar
                          key={i}
                          className={`${i < Math.floor(mainFeatured.rating) ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'} text-lg`}
                        />
                      ))}
                      <span className="ml-2 text-gray-700 dark:text-gray-300 font-medium">{mainFeatured.rating.toFixed(1)}</span>
                    </div>
                    ...
                    <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 space-x-4">
                      ...
                      {mainFeatured.views && (
                        <div className="flex items-center">
                          ...
                          <span>{mainFeatured.views} views</span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {secondaryFeatured.map((review, index) => (
              <FeaturedCard key={index} review={review} index={index} />
            ))}
          </div>
        )}
      </div>
